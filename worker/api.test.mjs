...
test('public catalogue, authenticated publication, password change and stale revision protection', async () => {
  ...
  assert.ok(initial.content.comboDeals.length > 0);
  assert.ok(initial.content.pizzaSupplements.length > 0);
  assert.equal(initial.revision, 0);
  const blocked = await call('content', { method: 'PUT', body: JSON.stringify(initial) });
  assert.equal(blocked.status, 401);
  assert.equal((await call('admin/login', { method: 'POST', headers: { Authorization: 'Bearer 1234', 'Content-Type': 'application/json' }, body: '{}' })).status, 200);
  assert.equal((await call('admin/login', { method: 'POST', headers: { Authorization: 'Bearer wrong', 'Content-Type': 'application/json' }, body: '{}' })).status, 401);
  initial.content.menuItems[0].nameFr = 'Pizza test';
  initial.content.comboDeals[0].titleFr = 'Formule test';
  initial.content.pizzaSupplements[0].nameFr = 'Supplément test';
  ...
  assert.equal((await call('admin/login', { method: 'POST', headers: { Authorization: 'Bearer 1234', 'Content-Type': 'application/json' }, body: '{}' })).status, 401);
  assert.equal((await call('admin/login', { method: 'POST', headers: { Authorization: 'Bearer 5678', 'Content-Type': 'application/json' }, body: '{}' })).status, 200);
  ...
  assert.equal(latest.content.comboDeals[0].titleFr, 'Formule test');
  assert.equal(latest.content.pizzaSupplements[0].nameFr, 'Supplément test');
