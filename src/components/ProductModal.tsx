...
import { CartItem, ComboDeal, Language, MenuItem, PizzaSupplement, SelectedSize, SelectedSupplement } from '../types';

interface ProductModalProps { item: MenuItem | null; comboDeals: ComboDeal[]; pizzaSupplements: PizzaSupplement[]; onClose: () => void; onAddToCart: (item: CartItem) => void; lang: Language; }
...
export const ProductModal: React.FC<ProductModalProps> = ({ item, comboDeals, pizzaSupplements, onClose, onAddToCart, lang }) => {
  ...
  const selectedCombo = comboDeals.find((combo) => combo.id === comboId);
  const standardBasePrice = item ? (item.prices ? (size ? item.prices[size] : 0) : item.singlePrice ?? 0) : 0;
  const basePrice = item?.promoPrice && item.promoPrice > 0 ? item.promoPrice : standardBasePrice;
  ...
  const availableSupplements = useMemo(() => {
    ...
    return pizzaSupplements.map((supplement) => ({ nameFr: supplement.nameFr, nameAr: supplement.nameAr, price: supplement.prices ? supplement.prices[size] : supplement.singlePrice ?? 0 }));
  }, [item, pizzaSupplements, size]);
  if (!item) return null;
    ...
    <div className="fixed inset-0 z-[65] flex items-end justify-center overflow-y-auto bg-black/80 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}>
      <div dir={lang === 'ar' ? 'rtl' : 'ltr'} onClick={(event) => event.stopPropagation()} className="relative max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl border border-white/10 bg-[#141414] shadow-2xl sm:rounded-3xl">
        ...
        <div className="space-y-5 p-5 sm:p-7">
          ...
          {item.categoryId !== 'boissons' && comboDeals.length > 0 && <fieldset><legend className="mb-2 text-xs font-black uppercase tracking-widest text-white/60">{lang === 'fr' ? 'Ajouter une formule' : 'إضافة عرض'}</legend><div className="space-y-2"><button type="button" onClick={() => setComboId(undefined)} className={`min-h-11 w-full rounded-xl border px-3 text-start text-xs ${!comboId ? 'border-[#FFD800] bg-[#FFD800]/10' : 'border-white/10 bg-white/5'}`}>{lang === 'fr' ? 'Sans formule' : 'بدون عرض'}</button>{comboDeals.map((combo) => <button type="button" key={combo.id} onClick={() => setComboId(combo.id)} className={`flex min-h-11 w-full items-center justify-between gap-2 rounded-xl border px-3 text-start text-xs ${comboId === combo.id ? 'border-[#FFD800] bg-[#FFD800]/10' : 'border-white/10 bg-white/5'}`}><span>{lang === 'fr' ? combo.titleFr : combo.titleAr}</span><strong className="shrink-0 text-[#FFD800]">+{combo.priceSupplement} DH</strong></button>)}</div></fieldset>}
